    ...
    <div class="max-w-md w-full bg-white p-8 rounded-lg shadow-lg">
      ...
      <form @submit.prevent="register" class="space-y-6">
        <div>
          ...
          <p v-if="emailExistsError" class="text-red-500 text-xs italic">
            {{ emailExistsError }}
          </p>
        </div>
        <div>
          ...
          <input
            ...
            required
          />
          <p v-if="usernameExistsError" class="text-red-500 text-xs italic">
            {{ usernameExistsError }}
          </p>
        </div>
        <div>
...
const register = () => {
  ...
  if (usernameExists) {
    usernameExistsError.value = "Користувач з таким ім'ям вже існує";
    return;
  } else {
